...
//	Here we look at using sync/atomic package for atomic counters accessed by multiple goroutines
...
func main() {
	...
	for i := 0; i < 50; i++ {
		...
		//	To atomically increment the counter we use AddUnit64, giving it the memory address of our ops counter with the & syntax
		go func() {
			for c := 0; c < 1000; c++ {
...
//	Moreover, we'd get data rae failures when running with the -race fla
